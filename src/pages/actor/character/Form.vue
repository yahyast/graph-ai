<template lang="pug">
    form.form-general(@submit.prevent="submit($event)")
        ...
        .form-general__body.w-full
            ...
            wysiwyg(v-model="model.character")
            
            //- span.field-message.text-red-500(v-if="$v.model.character.$error") This character is required.
...
export default {
    ...
    watch: {
        ...
        detail(newVal) {
            ...
            this.model.character = (newVal.character ? newVal.character : '');
        },
    },
    methods: {
        ...
        submit(e) {
            ...
            // update only
            this.$store.dispatch('actor/update', [this.id, this.model, 'character'])
            .then((resp) => {
    ...
    beforeRouteEnter (to, from, next) {
        next(vm => {
            console.log(to.name);
            if (to.name === 'CharacterOfActorForm') {
                vm.getData();
            }
        });
    },
